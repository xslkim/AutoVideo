...
15. **图片 / 视频模式**：API 生成图片时 visual 描述作为 prompt；本地图片 / 本地视频时 visual 描述仅作文档（详见 §3.2）
16. ⚠️ **图片模式底部留白**：`@visual: image` 生成的图片占满全屏，底部约 200px 会被字幕覆盖，prompt 中必须明确要求底部区域不放置重要内容（详见 §3.2）
17. **旁白每行一句**：TTS 逐行生成，空行忽略
18. ⚠️ **字幕单行长度上限**：中文 ≤ 50 字 / 英文 ≤ 70 字符；超长按标点拆行（详见 §4.4）
19. **高亮语法**：`**文字**` 在字幕高亮，TTS 只读文字本身；字面 `**` 用 `\*\*`
20. **资产路径**：图片用 `./` 开头的相对路径
21. **文件编码**：UTF-8
22. **`meta.md` 必填字段**：`title`；其余有默认值
