...
画面底部有一块**字幕安全区**（`subtitleSafeBottom` = `height × 50/1080`，1080p 下约 50px），字幕会渲染在此区域。编写视觉描述时需注意：
...
3. **写作建议**：在描述中明确写出"背景填满全屏"或"深色背景覆盖整个画面"，并确保最底部的元素与画面底部保持足够间距（≥ 50px）

```diff
- 深色背景，三个卡片从上到下排列
+ 深色背景 (#0d1117) 填满整个画面，三个卡片从上到下排列，
+ 最底部卡片距画面底部至少 50px（避让字幕区）。
```
...
- ⚠️ **底部留白避让字幕（重要）**：生成的图片会占满全屏，字幕直接覆盖在图片上方。画面底部约 **50px** 区域是字幕安全区，**不要在此区域放置重要文字、图标或关键内容**，否则会被字幕遮挡。在 prompt 中明确写出"画面底部 50px 区域留白，不放置任何文字或重要元素"或"所有重要内容集中在画面上半部分和中部"。

```
--- visual ---
一张科技风格的软件架构图，深色背景，蓝色 (#58a6ff) 连线连接各个模块，
模块用圆角矩形表示，白色文字标签，整体干净简洁。
画面底部 50px 区域留白，不放置任何文字或重要元素，避免被字幕遮挡。
```
...
13. ⚠️ **内容避让字幕区**：所有可见元素底边 ≤ `height - subtitleSafeBottom`（1080p 下约 50px），不与字幕重叠（详见 §3.1）
...
17. ⚠️ **图片模式底部留白**：`@visual: image` 生成的图片占满全屏，底部约 50px 会被字幕覆盖，prompt 中必须明确要求底部区域不放置重要内容（详见 §3.2）
